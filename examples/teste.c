...
void inserir(struct lista*, char, int, int);
void salvar(struct lista*);
int retirar(struct lista*, int, int);
		...
		int caractere = wgetch(stdscr);
			...
			switch(caractere) {
				...
				case KEY_F(3):
					...
					break;
				default:
					inserir(lista, (char)caractere, cursor->x, cursor->y);
...
int retirar(struct lista* lista, int x, int y) {
...
void inserir(struct lista* lista, char caractere, int x, int y) {
	struct lista* tmp = NULL;
